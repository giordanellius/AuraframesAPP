...
export async function POST(request: Request) {
  try {
    const { inputs, materialCost, laborCost, others, discountPercent, materialsMultiplier }: {
    ...
    const multiplier = materialsMultiplier ?? 2;
    const othersTotal = others.reduce((sum, o) => sum + o.amount, 0);
    const roundedMaterialAndLabour = roundToHalf(
      (materialCost * multiplier) + laborCost + othersTotal
    );
    const roundedSubtotal = roundedMaterialAndLabour;
    const roundedTotal = discountPercent > 0
      ? roundToHalf(roundedSubtotal * (1 - discountPercent / 100))
      : roundedSubtotal;
    ...
    // Build cost rows — others are folded into Materials & Labour
    const costRows = `<tr><td>Labour &amp; Materials</td><td>${formatCurrency(roundedMaterialAndLabour)}</td></tr>`;

    // Build Other item rows for Project Details
    const otherDetailRows = others
      .map((o) => `<tr><td>Other: ${o.title || 'Item'}</td><td>Included</td></tr>`)
      .join('');

    const discountBlock = discountPercent > 0
      ? `<tr>
          <td>Subtotal</td>
        ...
        if (result?.result) {
          ...
          return new NextResponse(pdfBuffer, {
            headers: {
              ...
              'Content-Disposition': 'attachment; filename="Aura_Custom_Quote.pdf"',
            },
          });
        } else {
          return NextResponse.json(
